...
def save_to_xlsx(tab, filename):
    verification = "=IF(AND(OR(B{0} =\"letters\",B{0} =\"figures\",B{0} =\"NamesHeightRelations\", B{0} =\"NamesAgeRelations\"),OR(C{0} = 2,C{0} = 3,C{0} = 4),OR(E{0} = 0,E{0} = 1),OR(F{0} = 0,F{0} = 1),OR(J{0} = 0,J{0} = 1),OR(L{0} = 0,L{0} = 1),OR(N{0} = 0,N{0} = 1),OR(O{0} = 0,O{0} = 1)),1, 0)"
    global_verification = "=SUM(R2: R500)"
    ...
    ws.append(
        ['BLOCK_NUMBER', 'SAMPLE_TYPE', 'N', 'NR', 'MEMORY', 'INTEGR', 'SHOW_TIME', 'RESP_TIME', 'MAX_TIME', 'FEEDB',
         'FEEDB_TIME', 'WAIT', 'EXP', 'FIX_TIME', 'LIST_VIEW', 'INSTRUCTION', '', global_verification])

    for idx, trial in enumerate(tab):
        if trial[1] == "instruction":
            trial_with_verification = trial[0:2] + ['']*4 + [trial[2]] + ['']*8 + [trial[-1]] + ['', 1]
        else:
